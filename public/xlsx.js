...
export async function exportOrders(orders) {
  ...
  const rows=[['Заказ','Дата','Статус','Клиент','Telegram','Telegram ID','Поступление','Артикул','Наименование','Количество','Цена, ₽','Сумма, ₽','Комментарий','Менеджер','Telegram менеджера']];
  for(const r of orders)for(const l of r.lines)rows.push([r.id,r.createdAt,r.status,r.user.name,r.user.username,r.user.id,r.shipmentTitle,l.sku,l.name,l.quantity,l.price/100,l.quantity*l.price/100,r.comment,r.manager?.name||'',r.manager?.username?'@'+r.manager.username:'']);
  ...
  const a=document.createElement('a');a.href=url;a.download='CR_Newsletters.xlsx';a.click();setTimeout(()=>URL.revokeObjectURL(url),30000);
}
