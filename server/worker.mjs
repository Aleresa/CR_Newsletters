...
export class NewsletterStore {
  ...
  async webhook(request) {
    ...
    if(message?.chat?.type==='private' && message.from) {
      ...
      if(text) await telegram(this.env,'sendMessage',{chat_id:message.chat.id,text,reply_markup:{inline_keyboard:[[{text:'Открыть поступления',url:this.env.MINI_APP_URL || 'https://t.me/CR_Newsletters_Bot'}]]}});
    }
...
export default {
  async fetch(request,env) {
    ...
    if(path.startsWith('/api/') || path==='/telegram/webhook') {
      ...
      return env.STORE.get(env.STORE.idFromName('cr-newsletters-v1')).fetch(request);
    }
